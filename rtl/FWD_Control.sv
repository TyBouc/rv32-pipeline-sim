    ...
    case (id_instr_opcode_ip)
      ...
      OPCODE_OP: begin // Register-Register ALU operation
        ...
        */
        ...
        // Forward for rs1 (fa_mux_op)
        if (ID_dest_rs1_ip != 0) begin //if its 0 its never changed anyway
          if ((ID_dest_rs1_ip === EX_MEM_dest_ip) && EX_MEM_RegWrite_en) begin
            fa_mux_op = EX_RESULT_SELECT;
          end else if ((ID_dest_rs1_ip == MEM_WB_dest_ip) && MEM_WB_RegWrite_en) begin
            fa_mux_op = WB_RESULT_SELECT;
          end
        end
        // Forward for rs2 (fb_mux_op)
        if (ID_dest_rs2_ip != 0) begin //if its 0 its never changed anyway
          if ((ID_dest_rs2_ip === EX_MEM_dest_ip) && EX_MEM_RegWrite_en) begin
            fb_mux_op = EX_RESULT_SELECT;
          end else if ((ID_dest_rs2_ip === MEM_WB_dest_ip) && MEM_WB_RegWrite_en) begin
